...
for tempr in [1e-22]:
    for dec_tempr in [1e-22]:
        ...
        if run_gpt:
                ...
                for model in ["microsoft/Phi-4-mini-instruct"]:
                    ...
                    for prompt_idx, prompt in tqdm(enumerate(cases), total=len(cases)):
                        ...
                        if prompt not in outputs:
                            ...
                            prompt += "\n\n**STA**\n" + completion + f"\n\nGiven the STA. Output YES if the case will be in the favour of {petitioners[prompt_idx]}, else NO"
            ...
            with open(f"ljp_{model_name}_with_defs_without_rr_without_chain_{tempr}_{dec_tempr}_{'uk' if 'UK' in root else 'in'}.json", 'w') as jsonf:
                json.dump(outputs, jsonf)
            # import sys
            # sys.exit(0)
                
        with open(f"ljp_{model_name}_with_defs_without_rr_without_chain_{tempr}_{dec_tempr}_{'uk' if 'UK' in root else 'in'}.json", 'r') as jsonf:
