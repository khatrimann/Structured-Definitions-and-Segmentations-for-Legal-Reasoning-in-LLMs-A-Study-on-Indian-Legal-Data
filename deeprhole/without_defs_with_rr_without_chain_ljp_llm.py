...
for tempr in [1e-22]:
    for dec_tempr in [1e-22]:
        ...
        if run_gpt:
                ...
                for model in ["microsoft/Phi-4-mini-instruct"]:
                    ...
                    for prompt_idx, prompt in tqdm(enumerate(cases), total=len(cases)):
                        ...
                        if prompt not in outputs:
                            ...
                            prompt = prompt[:prompt.find("Start with established laws")].strip()
            ...
            with open(f"ljp_{model_name}_without_defs_with_rr_without_chain_{tempr}_{dec_tempr}_{'uk' if 'UK' in root else 'in'}.json", 'w') as jsonf:
                json.dump(outputs, jsonf)
            # import sys
            # sys.exit(0)
                
        print(f"ljp_{model_name}_without_defs_with_rr_without_chain_{tempr}_{dec_tempr}_{'uk' if 'UK' in root else 'in'}.json")
        with open(f"ljp_{model_name}_without_defs_with_rr_without_chain_{tempr}_{dec_tempr}_{'uk' if 'UK' in root else 'in'}.json", 'r') as jsonf:
