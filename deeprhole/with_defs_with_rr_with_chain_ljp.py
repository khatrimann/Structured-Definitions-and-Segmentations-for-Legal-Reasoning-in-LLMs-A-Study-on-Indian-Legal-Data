    ...
    for dec_tempr in [1e-22]:
        ...
        for suffix in ["_preamble_plaintiff", "_plaintiff"]:
            ...
            if run_gpt:
                ...
                with open(f"ljp_{model_name}_with_defs_with_rr_with_chain1{suffix}_{tempr}_{dec_tempr}.json", 'w') as jsonf:
                    json.dump(outputs, jsonf)

                # import sys
                # sys.exit(0)
                    
            print(f"ljp_{model_name}_with_defs_with_rr_with_chain1{suffix}_{tempr}_{dec_tempr}.json")
            with open(f"ljp_{model_name}_with_defs_with_rr_with_chain1{suffix}_{tempr}_{dec_tempr}.json", 'r') as jsonf:
