...
for tempr in [1e-22]:
    for dec_tempr in [1e-22]:
        ...
        for suffix in ["_preamble_plaintiff", "_plaintiff"]:
            ...
            if run_gpt:
                outputs = {}
                for run in [1]:
                    for model in ["o3-mini-2025-01-31"]:
                ...
                with open(f"ljp_{model_name}_without_defs_without_rr_with_chain{suffix}_{tempr}_{dec_tempr}.json", 'w') as jsonf:
                    json.dump(outputs, jsonf)
                    
            print(f"ljp_{model_name}_without_defs_without_rr_with_chain{suffix}_{tempr}_{dec_tempr}.json")
            with open(f"ljp_{model_name}_without_defs_without_rr_with_chain{suffix}_{tempr}_{dec_tempr}.json", 'r') as jsonf:
