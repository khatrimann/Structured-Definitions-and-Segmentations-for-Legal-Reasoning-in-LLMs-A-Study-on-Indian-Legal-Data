    ...
    for dec_tempr in [1e-22]:
        ...
        for case_idx, case_ in enumerate(files):
            ...
            for line in file:
                ...
                if label in EXCLUDE:
                    filtered = filtered.replace(line, "")
                else:
        ...
        if run_gpt:
            ...
            with open(f"ljp_{model_name}_with_defs_with_rr_without_chain_{tempr}_{dec_tempr}_{'uk' if 'UK' in root else 'in'}.json", 'w') as jsonf:
                json.dump(outputs, jsonf)
            
            # import sys
            # sys.exit(0)
                
        print(f"ljp_{model_name}_with_defs_with_rr_without_chain_{tempr}_{dec_tempr}_{'uk' if 'UK' in root else 'in'}.json")
        with open(f"ljp_{model_name}_with_defs_with_rr_without_chain_{tempr}_{dec_tempr}_{'uk' if 'UK' in root else 'in'}.json", 'r') as jsonf:
